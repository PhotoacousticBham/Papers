...
for i in eachindex(R)
	...
	# Calculates the itfs
        itf[:,:,i] = pmap((λi, θi) -> itfpoint(R[i], hcav, n, ω, θi, λi), vec(λ[:,:,i]), vec(θ))
end
...
@save "tilt_vs_R.jld2" v sens θ itf λ
